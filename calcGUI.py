...
def button_press(num):
    ...
    equation_text.set(equation_text)

def equals():  # this block checks for button_press(equals)
    
    global equation_text

    try: # try is used in try...except blocks. It defines a block of code test if it contains any errors.
        # you can define differnt blocks for differnt error types, and blocks to execute  if nothing went wrong.
        total = str(eval(equation_text)) # pares the expression argument and evaluates it as a python error
...
button10 = Button(frame, text=0, height=4, width=9, font=35, command=lambda: button_press(0))
...
equals = Button(frame, text='=', height=4, width=9, font=35, command=lambda: button_press('='))
